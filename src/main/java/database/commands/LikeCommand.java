...
public enum LikeCommand {
    ...
    GET_BY_FROM_ID_AND_TO_ID("SELECT * FROM like_dislike WHERE from_user_id = ? AND to_user_id = ?"),
    DELETE("DELETE FROM like_dislike WHERE id = ? RETURNING id"),
    GET_COUNT_FROM_ID("SELECT count(*) FROM like_dislike WHERE from_user_id = ?"),
